#!/usr/bin/env python3
"""
Generates reference test paths for real-world validation of the nav2 global
planner (config/nav2_planning_params.yaml) that was tuned in simulation.
...
To use in the real world: pace off / mark this frame on the ground (e.g.
with cones or GPS waypoints) starting at the rover's actual launch point and
heading, then either drive the CSV waypoints directly (they're already in
the `[[x, y], ...]` format used by scripts/rover_controller_pure_pursuit.py)
or feed them to nav2 as a sequence of goal poses.

Path sizes are scaled to the *same rover as in simulation* (the 6-wheel
rocker-bogie in Mars-rover/rover_description), not car-scale distances.
Dimensions below come from forward-kinematics on
Mars-rover/rover_description/urdf/{aries_base,left_link,right_link}.xacro at
the neutral (zero rocker/bogie angle) pose:
  - wheel-center wheelbase   ~0.571 m (front/rear wheel x-extent)
  - wheel-center track width ~0.715 m (matches wheel_separation=0.7095 in
    rover_controllers/config/diff_controller.yaml)
  - wheel radius              0.110 m
giving an overall ground footprint of roughly 0.8 m x 0.8 m (wheelbase/track
plus wheel radius overhang). All path dimensions below are expressed as
multiples of that footprint so the course scales if the rover does.
