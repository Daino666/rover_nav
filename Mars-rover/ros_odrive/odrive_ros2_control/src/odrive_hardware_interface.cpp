...
struct Axis {
    Axis(SocketCanIntf* can_intf, uint32_t node_id, double direction = 1.0) :
        can_intf_(can_intf), node_id_(node_id), direction_(direction) {}
    ...
    SocketCanIntf* can_intf_;
    uint32_t node_id_;
    // +1 or -1, set per-joint via the "direction" hardware param. Corrects
    // for axes mounted as a mirror image of the other side (e.g. left vs.
    // right wheel pods): both the outgoing command (write()) and the
    // encoder feedback (on_can_msg() below) are mirrored the same way on
    // that physical axis, so both need the same sign correction to line
    // up with the URDF's symmetric joint convention.
    double direction_;
    ...
    double pos_estimate_ = 0.0; // [rad] — 0 until first CAN frame arrives
...
CallbackReturn ODriveHardwareInterface::on_init(const hardware_interface::HardwareInfo& info) {
    ...
    for (auto& joint : info_.joints) {
        double direction = joint.parameters.count("direction") ? std::stod(joint.parameters.at("direction")) : 1.0;
        axes_.emplace_back(&can_intf_, std::stoi(joint.parameters.at("node_id")), direction);
    }
...
return_type ODriveHardwareInterface::write(const rclcpp::Time&, const rclcpp::Duration&) {
    for (auto& axis : axes_) {
        ...
        // -direction_, not direction_: on_can_msg() below was validated against
        // actual observed physical motion (positive odom == genuine forward
        // travel), independent of what cmd_vel sign produced that motion. This
        // command side was separately found to need the opposite sign from
        // that validated feedback mapping — confirmed via pure_pursuite_testtb.py,
        // which only ever publishes non-negative linear_x, so if it drove
        // backward, positive cmd_vel.x was driving the wheels backward.
        if (axis.pos_input_enabled_) {
            Set_Input_Pos_msg_t msg;
            msg.Input_Pos = (-axis.direction_ * axis.pos_setpoint_) / (2 * M_PI);
            msg.Vel_FF = axis.vel_input_enabled_ ? (-axis.direction_ * axis.vel_setpoint_ / (2 * M_PI)) : 0.0f;
            msg.Torque_FF = axis.torque_input_enabled_ ? (-axis.direction_ * axis.torque_setpoint_) : 0.0f;
            axis.send(msg);
        } else if (axis.vel_input_enabled_) {
            Set_Input_Vel_msg_t msg;
            msg.Input_Vel = (-axis.direction_ * axis.vel_setpoint_) / (2 * M_PI);
            msg.Input_Torque_FF = axis.torque_input_enabled_ ? (-axis.direction_ * axis.torque_setpoint_) : 0.0f;
            axis.send(msg);
        } else if (axis.torque_input_enabled_) {
            Set_Input_Torque_msg_t msg;
            msg.Input_Torque = -axis.direction_ * axis.torque_setpoint_;
            axis.send(msg);
        } else {
    ...
    switch (cmd) {
        case Get_Encoder_Estimates_msg_t::cmd_id: {
            if (Get_Encoder_Estimates_msg_t msg; try_decode(msg)) {
                // -direction_, not direction_: validated directly against observed
                // physical motion (positive odom == genuine forward travel), which
                // is independent of whatever cmd_vel sign was used to produce that
                // motion. See the comment in write() — the command side needed its
                // own, separately-determined negation once the actual cmd_vel sign
                // convention was pinned down via pure_pursuite_testtb.py.
                pos_estimate_ = -direction_ * msg.Pos_Estimate * (2 * M_PI);
                vel_estimate_ = -direction_ * msg.Vel_Estimate * (2 * M_PI);
            }
        } break;
        case Get_Torques_msg_t::cmd_id: {
            if (Get_Torques_msg_t msg; try_decode(msg)) {
                torque_target_ = -direction_ * msg.Torque_Target;
                torque_estimate_ = -direction_ * msg.Torque_Estimate;
            }
        } break;
