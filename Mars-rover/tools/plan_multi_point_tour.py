#!/usr/bin/env python3
"""
Give it a list of named points; it orders them nearest-first (greedy nearest-
neighbor, starting from START), optionally returns to START at the end, and
asks the real Nav2 stack (planner_server + smoother_server, already running
via `ros2 launch rover_nav nav2_planning.launch.py`) for ONE continuous
smoothed path through all of them -- not a separate re-plan per leg.

The only custom logic here is the nearest-neighbor ORDERING (plain sorting,
~15 lines) -- all the actual pathfinding/smoothing is Nav2's
ComputePathThroughPoses + SmoothPath, same as plan_path.py used for a single
point. Pure pursuit / actually driving the path is intentionally not wired
in here -- you already have that part.

HOW TO USE: make sure the Nav2 planning stack is running first:
  ros2 launch rover_nav nav2_planning.launch.py
Then edit the PARAMETERS block below and run:
  python3 plan_multi_point_tour.py

POINTS can be survey point names ("W3", "L5", ...) or raw (x, y) tuples,
mixed freely.
"""
...
START = "S1"
# Starting point: a survey point name or a raw (x, y) tuple in world meters.

POINTS = ["W6", "W5", "W7", "W8"]
# The points to visit, in ANY order -- they'll be automatically sorted
# nearest-first starting from START. Names or raw (x, y) tuples, mixed OK.

RETURN_TO_START = True
# If True, appends START back onto the end of the route after the last
# point, so the final path is a full loop.
...
OUT_WAYPOINTS = os.path.join(DEFAULT_OUT_DIR, "marsyard2026_tour_waypoints.csv")
OUT_VIZ = os.path.join(DEFAULT_OUT_DIR, "marsyard2026_tour_preview.png")
...
def resolve(spec, survey_points):
    ...
    return f"({spec[0]:.2f},{spec[1]:.2f})", tuple(spec)
...
def save_preview(route_labels, route_xy, path_world, survey_points, out_viz):
    ...
    yaml_path = os.path.join(DEFAULT_OUT_DIR, "marsyard2026_occupancy.yaml")
    with open(yaml_path) as f:
        meta = _yaml.safe_load(f)
    pgm = np.array(Image.open(os.path.join(DEFAULT_OUT_DIR, meta["image"])))
    res = meta["resolution"]
