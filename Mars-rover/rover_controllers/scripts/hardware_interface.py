...
# ── Wheel layout (matches rover_control.xacro node_id assignment) ────────────
# All-left then all-right:  L_1=0  L_2=1  L_3=2  R_1=3  R_2=4  R_3=5
AXIS_LABELS = {
    0: "Left-Front ",
    1: "Left-Mid   ",
    2: "Left-Rear  ",
...
class RoverHardwareChecker(Node):
    def __init__(self):
        ...
        self.require_all_axes   = self.get_parameter("require_all_axes").value
        self.check_joystick     = self.get_parameter("check_joystick").value
    ...
    def _check_status(self):
        ...
        if not self.initial_check_done or status_changed or self.manual_check_requested:
            self.manual_check_requested = False
            self._print_status(axis_node_up, axis_has_data, axis_cl, axis_err, joystick_ok)
    ...
    def _print_status(self, axis_node_up, axis_has_data, axis_cl, axis_err, joystick_ok):
        W = 64
        G = self.GREEN
        R = self.RED
        Y = self.YELLOW
        ...
        all_cl         = all(axis_cl[i] for i in range(NUM_AXES) if axis_has_data[i]) \
                         if any(axis_has_data) else True
        any_err        = any(axis_err)
        overall_ok     = all_node_up and all_responding and not any_err and \
            (all_cl if self.require_closed_loop else True)
        ...
        # ── Per-axis rows ─────────────────────────────────────────────────────
        print(f"\n{B}  ODrive Axes  (interleaved L/R per axle — even=Left, odd=Right):{RST}", flush=True)
